...
### The storage system
##### 要約
- ストレージ（リポジトリ）は、クリーンアーキテクチャの3番目の層、外部システムにあります。このレイヤの要素は、インターフェイスを介して内部要素によってアクセスされます。
- クリーンアーキテクチャのリポジトリによって提供される抽象化のレベルは、フレームワーク内のORMまたはSQLAlchemyのようなツールによって提供される抽象化レベルよりも高いです。
- リポジトリは、アプリケーションが必要とするエンドポイントのみを提供し、アプリケーションが実装する特定のビジネス上の問題に合わせて調整されたインターフェースを備えています。
- ？ SQLAlchemyはSQLデータベースへのアクセスを抽象化するための素晴らしいツールです。そのため、リポジトリの内部実装では、たとえばPostgreSQLデータベースへのアクセスに使用できます。しかし、層の外部APIはSQLAlchemyによって提供されているものではありません。 APIは、ユースケースがデータを取得するために呼び出す一連の機能を減らしたものです。内部実装では、生のSQLクエリからRabbitMQネットワークを介した複雑なリモート呼び出しシステムまで、さまざまなソリューションを使用して同じ目的を達成できます。
- リポジトリの非常に重要な機能は、ドメインモデルを返すことができることです。これは、フレームワークORMが通常行うことと一致しています。 
- 3番目の層の要素は、内部層で定義されているすべての要素にアクセスできます。つまり、ドメインモデルとユースケースはリポジトリから直接呼び出して使用できます。
- 私たちは自分たちのニーズによりよく合うシステムでリポジトリを自由に実装することができます。
- このクラスが実際のデータベースや他のストレージタイプのラッパーであることは容易に想像できます。コードはより複雑になるかもしれませんが、リポジトリの構造は同じで、単一のパブリックメソッドリストがあります。データベースリポジトリについては、後の章で詳しく説明します。

### A command line interface
##### 要約
- これまでのところ、ドメインモデル、シリアライザ、ユースケース、およびリポジトリを作成しましたが、すべてをまとめるシステムがまだ不足しています。
- このシステムは、ユーザから呼び出しパラメータを取得し、リポジトリでユースケースを初期化し、リポジトリからドメインモデルを取得するユースケースを実行し、それらをユーザに返す必要があります。
- クリーンアーキテクチャの威力は、外部システムがプラガブルであるということです。つまり、使用したいシステムの詳細についての決定を延期することができます。この場合、システムにクエリを実行し、ストレージシステムに含まれる部屋のリストを取得するためのインターフェイスをユーザーに提供します。最も簡単な方法はコマンドラインツールです。
- 後でRESTエンドポイントを作成し、それをWebサーバーを介して公開します。
- cli.pyファイルを作成します。これは、そこに含まれるすべてのドメインモデルについてストレージを照会するだけなので、実行するための特別なオプションを必要としない単純なPythonスクリプトです。
- cli.pyファイルのMemRepoクラスが空のリストで初期化されているので、これは正しいです。私たちが使っているシンプルなインメモリストレージは永続性がないので、作成するたびにデータを読み込む必要があります。これは、ストレージ層を単純にするために行われましたが、ストレージが適切なデータベースであれば、コードのこの部分はそれに接続しますが、そこにデータをロードする必要はないということを覚えておいてください。スクリプトの重要な部分は3行です
- これはリポジトリを初期化し、それを使ってユースケースを初期化し、そして後者を実行します。これは一般的にあなたがあなたがそれに接続するどんな外部システムでもあなたのきれいなアーキテクチャを使うことになる方法です。他のシステムを初期化し、ユースケースを初期化し、そして結果を収集します。デモンストレーションのために、ファイルにデータを定義してリポジトリにロードしましょう。
- 繰り返しますが、これは私たちのストレージの些細な性質によるもので、システムのアーキテクチャーによるものではないことを忘れないでください。リポジトリがドメインモデルを返すときに印刷命令を変更したので、それを印刷すると<rentomatic.domain.room.Roomオブジェクトat 0x7fb815ec04e0>のような文字列のリストが表示されるので、あまり役に立ちません。今コマンドラインツールを実行すると、以前よりも豊富な結果が得られます。
