...
- [このクラス](https://github.com/esaki01/clean-architecture-python/blob/master/app/repository/memrepo.py)が実際のデータベースのラッパークラスであることは容易に想像できます。コードはより複雑になるかもしれませんが、単一のパブリックメソッドがあるという点では同じです。データベースリポジトリについては、後の章で詳しく説明します。
...
### Conclusions
##### 要約
私たちが作成した階層化アーキテクチャの力を皆さんに理解して頂けているなら幸いです。私たちはモデルのリストを「正しく」出力するために多くのコードを書きましたが、それらのコードは簡単に拡張や修正ができます。それらはまた十分にテストされています。今回のユースケースでは、実装が（意図的に）非常に単純なものとなっています。入力を必要とせず、エラー状態を返すこともできないため、作成したコードは入力の検証とエラー管理を完全に無視しました。しかしながら、これらのトピックは非常に重要であるため、クリーンなアーキテクチャがそれらをどのように扱うことができるのかを議論する必要があります。
