...
- クリーンアーキテクチャの良いところは、external systemsがpluggableであるということです。つまり、使用したいシステムの詳細についての決定を延期することができます。今回の場合、システムにクエリを実行し、ストレージシステムに格納されている含まれるRoom listを取得するためのインターフェイスをユーザーに提供します。最も簡単な方法はコマンドラインツールです（後でRESTエンドポイントを作成し、それをWebサーバーを介して公開します）。
...
- cli.pyは、リポジトリを初期化し、ユースケースを初期化、実行します。一般的に、接続するexternal systemsがどんなものであっても、このようにしてクリーンアーキテクチャを利用することになります。
...
### Conclusions
##### 要約
私たちが作成した階層化アーキテクチャの力を皆さんに理解して頂けているなら幸いです。私たちはモデルのリストを「正しく」出力するために多くのコードを書きましたが、それらのコードは簡単に拡張や修正ができます。それらはまた十分にテストされています。今回のユースケースでは、実装が（意図的に）非常に単純なものとなっています。入力を必要とせず、エラーの状態を返すこともできないため、作成したコードは入力の検証とエラー管理を完全に無視しました。しかしながら、これらのトピックは非常に重要であるため、クリーンなアーキテクチャがそれらをどのように扱うことができるのかを議論する必要があります。
