...
- SQLAlchemyはSQLデータベースへのアクセスを抽象化するための素晴らしいツールです。そのため、リポジトリの内部実装では、たとえばPostgreSQLデータベースへのアクセスに使用できます。
...
- ユーザから入力パラメータを取得し、リポジトリでユースケースを初期化し、リポジトリからドメインモデルを取得するユースケースを実行し、それらをユーザに返す必要があります。
...
- リポジトリ直下にcli.pyを作成します。ドメインモデルについてストレージを照会するだけの単純なPythonスクリプトです。
