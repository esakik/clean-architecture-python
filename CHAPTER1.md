...
### Layers and data flow
##### 要約
- クリーンアーキテクチャは、システムを様々なカテゴリに分類し、割り当てられたカテゴリは、そのカテゴリ内の特定の場所に配置されます。
...
##### わからなかったこと
- 「Use Casesはお互いに呼び出すこともでき、他のより単純なものをまとめた複雑なユースケースを作成するのが一般的です」という文脈で、そうするとUse Casesパッケージ（のようなもの）が膨らんでしまうのではないかという疑問点が湧きました。
...
#### APIs and shades of grey
##### 要約
- 「何がどこにあるのか、そして何故なのか」を知ることが非常に重要でありますが、多くの場合で白黒はっきりつけることは難しく、「グレー」な問題となりやすいです。何か与えられた場所に置く理由を正当化するのはあなた次第です。
 - データフローを壊してしまうと、基本的に構造全体が無効になります。できる限り、壊してしまわないように努力すべきですが、現実的には、もし、コストの面で規則を破らなければならなくなった場合は、あなたがそれをなぜしたかを説明するあなたのコードとあなたのドキュメンテーションのに大きな警告を書き、他のプログラマーにはっきりとそれを起こしたことを伝えるべきです。
   - （例として、ユースケースはインターフェースを介してストレージ層にアクセスすることですが、これは遅すぎることがわかります。次に、使用している特定のデータベースのAPIに直接アクセスすることにしましたが、現在は内部層（ユースケース）が外部の層（外部インターフェース）にアクセスしているため、データフローが中断されます。将来、誰かがあなたが使用している特定のデータベースを別のデータベースと交換したい場合、新しいデータベースが同じAPIエントリポイントに同じデータを提供することはおそらくないので、彼らはこれを認識しなければなりません。）
